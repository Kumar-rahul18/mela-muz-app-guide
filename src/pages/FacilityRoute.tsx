...
const FacilityRoute = () => {
  ...
  const renderSpecialPages = () => {
    if (type === 'mela-route') {
      const melaRouteFacility = facilities.find(fac => fac.facility_type === 'mela-route' && fac.google_maps_link);
          ...
          {melaRouteFacility ? (
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              ...
              <div className="relative w-full" style={{ paddingBottom: '56.25%', height: 0 }}>
                <iframe
                  src={melaRouteFacility.google_maps_link}
                  allowFullScreen
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                  title="Mela Route Map"
                  className="absolute top-0 left-0 w-full h-full rounded-xl border border-gray-200"
                />
              </div>

              <div className="mt-4 flex justify-center">
                <Button
                  onClick={() => window.open(melaRouteFacility.google_maps_link, '_blank')}
                  className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-full"
                >
                  🔗 Open Full Map
                </Button>
            ...
            <div className="text-center py-12 text-gray-500">No map available</div>
          )}
        </div>
      );
    }
